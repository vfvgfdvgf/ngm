...
(function () {
  ...
  function setupSmartPrefetch() {
    if (!("IntersectionObserver" in window) || !document.head) return;
    ...
    const links = Array.from(document.querySelectorAll("a[href]")).filter((link) => {
      ...
      return href.startsWith("/") && !href.startsWith("//") && !link.hasAttribute("download");
    });

    links.forEach((link) => {
      const queuePrefetch = () => prefetchDocument(link.href);
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (!entry.isIntersecting) return;
          prefetchDocument(entry.target.href);
          observer.unobserve(entry.target);
        });
      },
      {
    ...
    links.slice(0, 18).forEach((link) => observer.observe(link));
  }
...
(function () {
  const desktopMode = window.matchMedia("(min-width: 961px)").matches;
  const root = document.documentElement;
  const body = document.body;
  const topbar = document.querySelector("[data-topbar]");
  const playerShell = document.querySelector("[data-player-shell]");
  ...
  root.setAttribute("data-device", desktopMode ? "desktop" : "mobile");
  body.setAttribute("data-device", desktopMode ? "desktop" : "mobile");
  body.classList.remove("sidebar-open");

  if (topbar) topbar.classList.add(desktopMode ? "is-desktop-ui" : "is-mobile-ui");
  if (playerShell) playerShell.classList.add(desktopMode ? "is-desktop-player" : "is-mobile-player");
